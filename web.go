// Package web provides the main web server implementation for the Spider Panel,
...
// Server represents the main web server for the Spider Panel with controllers, services, and scheduled jobs.
type Server struct {
...
func (s *Server) initRouter() (*gin.Engine, error) {
	...
	// Mount subscription routes on the same Gin engine (single-port Railway mode)
	s.subServer = sub.NewServer()
	if err := s.subServer.RegisterRoutes(engine, htmlFS, assetsFS); err != nil {
		logger.Warning("sub: failed to register routes:", err)
	}
...
// GetHttpServer returns the underlying HTTP server for external middleware.
func (s *Server) GetHttpServer() *http.Server {
	return s.httpServer
}

// GetSubServer returns the subscription server for route access.
func (s *Server) GetSubServer() *sub.Server {
	return s.subServer
}

// RestartXray restarts the Xray process (used by signal handler).
