// Package main is the entry point for the Spider web panel application.
...
// runWebServer initializes and starts the single web server for the Spider panel.
// Subscription routes are mounted INSIDE this server (single-port Railway mode).
func runWebServer() {
	...
	for {
		...
		switch sig {
		case syscall.SIGHUP:
			logger.Info("Received SIGHUP signal. Restarting server...")
...
// migrateDb performs database migration operations for the Spider panel.
func migrateDb() {
...
// main is the entry point of the Spider application.
...
func main() {
	...
	case "setting":
		...
		if !enabletgbot && tgbottoken != "" {
			updateTgbotEnableSts(false)
		}
