...
func (s *SubService) genVlessLink(inbound *model.Inbound, email string) string {
	...
	address := s.resolveInboundAddress(inbound)
	port := inbound.Port
	// Client-level external proxy overrides the inbound address/port
	if client.ExternalProxy != "" {
		address = client.ExternalProxy
	}
	if client.ExternalPort != 0 {
...
func (s *SubService) getBaseSchemeAndHost(requestScheme, requestHostWithPort string) (string, string) {
	...
	// Build host:port — always include port for sub server binding, but omit it from URLs
	_ = subPort // port is used only for binding, not in URLs
	hostWithPort := fmt.Sprintf("%s", subDomain)
